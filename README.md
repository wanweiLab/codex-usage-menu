...
<p align="center">一个轻量的 macOS 菜单栏 Codex 额度监控与健康提醒工具。</p>
...
- 展示周额度、短周期额度和重置时间
- 支持自定义状态栏前缀，留空则只显示周额度
- 支持独立配置久坐提醒和喝水提醒
- 屏幕中央置顶弹窗显示 60 秒倒计时，支持自动或手动关闭
- 启动时读取一次账号套餐和脱敏邮箱
- 每 5 分钟自动刷新，支持手动刷新
...
点击 Codex Pulse 面板底部的齿轮，可以修改周额度前面的菜单栏文字。输入时会实时预览，点击“恢复默认”可还原为 `CodeX`。
...
前缀最多 8 个字符，超过后输入框会直接截断。这个设置仅保存在当前 Mac 的 `UserDefaults` 中，不会发送给 Codex、OpenAI 或任何第三方。

## 久坐和喝水提醒

点击 Codex Pulse 面板底部的齿轮，在“健康提醒”中分别配置：

- 久坐提醒：默认每 60 分钟提醒一次
- 喝水提醒：默认每 45 分钟提醒一次
- 两个提醒均支持 `1–720` 分钟的独立间隔
- 点击“测试”可立即预览对应提醒弹窗

提醒会以正方形置顶弹窗显示在当前屏幕中央，圆环从 60 秒开始倒计时。倒计时结束后弹窗自动关闭，也可以点击右上角叉号或“关闭”按钮手动关闭。

提醒由 Codex Pulse 在本机计时，不使用网络，也不需要 macOS 通知权限。Codex Pulse 退出后不会继续提醒；重新启动后会恢复已开启的提醒并从头计时。
